...
with train_summary_writer.as_default():
    for epoch in range(config['epochs']):
        ...
        for index, value in enumerate(train_dataset):
            xis, xjs = value
        ...
        time_by_epoch = ((datetime.datetime.now() - t_start).total_seconds()/ 60.0)
        print("Total time per epoch:", time_by_epoch)
        check_point(model)
